...
export interface AnnouncementChoices {
  types: { value: string; label: string }[];
  priorities: { value: string; label: string }[];
}

export async function getAnnouncementChoices(
  accessToken: string,
): Promise<AnnouncementChoices> {
  if (!accessToken) throw new Error("Authentication token required");
  return fetchAPI("/v1/announcements/choices", {
    method: "GET",
    headers: { Authorization: `Bearer ${accessToken}` },
  });
}
...
export async function getAnnouncements(
  ...
  return fetchAPI(`/v1/announcements/${qs ? `?${qs}` : ""}`, {
    method: "GET",
    headers: { Authorization: `Bearer ${accessToken}` },
  });
}
...
export async function getAnnouncementById(
  ...
  if (!accessToken) throw new Error("Authentication token required");
  return fetchAPI(`/v1/announcements/${id}/`, {
    method: "GET",
    headers: { Authorization: `Bearer ${accessToken}` },
  });
}
...
export async function createAnnouncement(
  ...
  if (!accessToken) throw new Error("Authentication token required");
  return fetchAPI("/v1/announcements/", {
    method: "POST",
    headers: { Authorization: `Bearer ${accessToken}` },
    body: JSON.stringify(data),
  });
